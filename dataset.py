...
class BaseVoxelDataset(Dataset):
    ...
    def __getitem__(self, index, seed=None):
        ...
        if len(xs) < 3:
            ...
            # print('BaseVoxel2', torch.max(voxel), torch.min(voxel), torch.mean(voxel), voxel.dtype, voxel.shape)
        # if frame.dtype == np.uint16:
        #     frame = frame[30:250, 30:280]
        #     voxel = voxel[:, 30:250, 30:280]
        ...
        if self.voxel_method['method'] == 'between_frames':
            ...
            # print('BaseVoxel', torch.max(voxel), torch.min(voxel), torch.mean(voxel), voxel.dtype, voxel.shape)
    ...
    def transform_frame(self, frame, seed):
        ...
        """
        if frame.dtype == np.uint16:
            # frame = standardization(frame)
            # scale = 1.
            scale = 65535.
        else:
            scale = 255.
        frame = torch.from_numpy(frame/scale).float().unsqueeze(0)
        if self.transform:
            random.seed(seed)
...
class PreprocessedH5Dataset(Dataset):
    def __init__(self, data_path, transforms={}, sensor_resolution=None, num_bins=5,
                 voxel_method=None, max_length=None, combined_voxel_channels=True,
                 filter_hot_events=False):
        super(PreprocessedH5Dataset, self).__init__()
        self.data_path = data_path
        transforms_list = [eval(t)(**kwargs) for t, kwargs in transforms.items()]
        if len(transforms_list) == 0:
            self.transform = None
    ...
    def __getitem__(self, index, seed=None):
        ...
        assert 0 <= index < self.__len__(), "index {} out of bounds (0 <= x < {})".format(index, self.__len__())
        seed = random.randint(0, 2 ** 32) if seed is None else seed
        frame = self.h5_file['frame'][index]
        frame = self.transform_frame(frame, seed)
        flow = torch.zeros((2, frame.shape[-2], frame.shape[-1]), dtype=frame.dtype, device=frame.device)
        voxel = self.h5_file['events'][index]
        voxel = self.transform_voxel(voxel, seed).float()
        timestamp = torch.tensor(self.h5_file['timestamp'][index], dtype=torch.float64)
        data_source_idx = self.h5_file['data_source_idx'][index].astype('int16')
        dt = self.h5_file['dt'][index]
...
class PreprocessedSequenceH5Dataset(Dataset):
    def __init__(self, data_path, sequence_length, step_size, transforms={}):
        super(PreprocessedH5Dataset, self).__init__()
        self.data_path = data_path
        ...
        with h5py.File(data_path, 'r') as f:
            self.length = f['frame'].shape[0]
        if self.L >= self.length:
            self.length = 0
        else:
            self.length = (self.length - self.L) // self.step_size + 1
    ...
    def __getitem__(self, index, seed=None):
        ...
        assert 0 <= index < self.__len__(), "index {} out of bounds (0 <= x < {})".format(index, self.__len__())
        seed = random.randint(0, 2 ** 32) if seed is None else seed
        s = index * self.step_size
        e = s + self.L
        frame = self.h5_file['frame'][s:e]
        frame = self.transform_frame(frame, seed)
        flow = torch.zeros((self.L, 2, frame.shape[-2], frame.shape[-1]), dtype=frame.dtype, device=frame.device)
        voxel = self.h5_file['events'][s:e]
        voxel = self.transform_voxel(voxel, seed).float()
        timestamp = torch.tensor(self.h5_file['timestamp'][s:e], dtype=torch.float64)
        data_source_idx = self.h5_file['data_source_idx'][s:e].astype('int16')
        dt = self.h5_file['dt'][s:e]
        item = {'frame': frame,
                'flow': flow,
                'events': voxel,
                'timestamp': timestamp,
...
class ConsecutiveH5Dataset(Dataset):
    """Load consecutive DynamicH5Dataset and the central ground truth
    """
    def __init__(self, data_root, num_inputs, dataset_type='DynamicH5Dataset', dataset_kwargs={}):
        self.num_inputs = num_inputs
        self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)
        self.central_idx = num_inputs // 2
        self.length = len(self.dataset) - num_inputs + 1
    ...
    def __getitem__(self, i, seed=None):
        ...
        events = []
        item = self.dataset.__getitem__(i+self.central_idx, seed)
        # print(item['events'].shape, item['frame'])
        for idx in range(self.num_inputs):
            j = i + idx
            events.append(torch.unsqueeze(self.dataset.__getitem__(j, seed)['events'], 0))
        item['events'] = torch.cat(events, 0)
        # print(torch.max(events[0]), torch.min(events[0]), torch.mean(events[0]), events[0].dtype, events[0].shape)
        # import ipdb; ipdb.set_trace()
        # print('CH5', torch.max(item['events']), torch.min(item['events']), torch.mean(item['events']), item['events'].dtype, item['events'].shape)
        return item


class MultipleOutputH5Dataset(Dataset):
    """Load consecutive DynamicH5Dataset and the corresponding ground truth
    """
    def __init__(self, data_root, num_inputs, dataset_type='DynamicH5Dataset', dataset_kwargs={}):
        self.num_inputs = num_inputs
        self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)
        self.central_idx = num_inputs // 2
        self.length = (self.dataset.length) // self.num_inputs
    ...
    def __getitem__(self, i, seed=None):
        assert(i >= 0)
        assert(i < self.length)
        start_idx = i * (self.num_inputs - 4)
        events = []
        frames = []
        item = self.dataset.__getitem__(start_idx+self.central_idx, seed)
        for idx in range(self.num_inputs):
            j = start_idx + idx
            events.append(torch.unsqueeze(self.dataset.__getitem__(j, seed)['events'], 0))
            frames.append(torch.unsqueeze(self.dataset.__getitem__(j, seed)['frame'], 0))
        item['events'] = torch.cat(events, 0)
        item['frame'] = torch.cat(frames, 0)
        return item
...
class SequenceDataset(Dataset):
    ...
    def __init__(self, data_root, sequence_length, dataset_type='MemMapDataset',
            step_size=None, neighbor_length=3, proba_pause_when_running=0.0,
        ...
        if self.dataset_type in ['ConsecutiveH5Dataset', 'EventSRDataset']:
            self.dataset = eval(dataset_type)(data_root, neighbor_length, dataset_kwargs=dataset_kwargs)
        else:
            self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)
    ...
    def __getitem__(self, i):
        ...
        if self.dataset_type == 'ConsecutiveH5Dataset':
            # print('Sequence', torch.max(item['events']), torch.min(item['events']), torch.mean(item['events']), item['events'].dtype, item['events'].shape)
            return sequence
        # add hot pixels
...
class ConsistencyDataset(Dataset):
    """Load consecutive two frames and the event streams bewtween the two frames"""
    def __init__(self, data_root, dataset_type='DynamicH5Dataset', dataset_kwargs={}):
        self.dataset = eval(dataset_type)(data_root, **dataset_kwargs)
        self.length = self.dataset.length - 1
    ...
    def __getitem__(self, i):
        ...
        # in the same way
        seed = random.randint(0, 2**32)
        frame1 = self.dataset.__getitem__(i, seed)['frame']
        frame2 = self.dataset.__getitem__(i+1, seed)['frame']
        events = self.dataset.__getitem__(i+1, seed)['events']
        out = {
            'frame1': frame1,
            'frame2': frame2,
            'events': events
